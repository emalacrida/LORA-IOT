...
#ifdef Device_A
  pCFG->ADDH = DEVICE_A_ADDR_H; //0x05
  pCFG->ADDL = DEVICE_A_ADDR_L; //0x01
#else
  pCFG->ADDH = DEVICE_B_ADDR_H; //0x05
  pCFG->ADDL = DEVICE_B_ADDR_L; //0x01
#endif

  pCFG->OPTION_bits.trsm_mode =TRSM_FP_MODE; //0x01
  pCFG->OPTION_bits.tsmt_pwr = TSMT_PWR_10DB; //0x03

  STATUS = SleepModeCmd(W_CFG_PWR_DWN_SAVE, (void* )pCFG); // 0xC0
